    ...
    if (dryRun) {
      ...
      try {
        ...
        if (verbose && keys.length > 0) {
          console.log('   Sample keys:', keys.slice(0, 10).join(', ') + (keys.length > 10 ? '...' : ''));
        }
      } catch (error) {
...
async function flushFileCache(dryRun = false, verbose = false) {
  ...
  const filesToClear = [
    ...
    path.join(dataDir, 'violations.json')
  ];
  ...
  if (dryRun) {
    ...
    for (const filePath of filesToClear) {
      if (fs.existsSync(filePath)) {
        ...
        console.log(`   Would delete: ${path.basename(filePath)} (${(stats.size / 1024).toFixed(1)} KB)`);
      }
    }
    return true;
  }
  ...
  for (const filePath of filesToClear) {
    try {
      if (fs.existsSync(filePath)) {
        ...
        deletedCount++;
        if (verbose) {
          console.log(`   ✅ Deleted ${path.basename(filePath)} (${(stats.size / 1024).toFixed(1)} KB)`);
        }
      }
    } catch (error) {
      if (verbose) {
