...
describe('User Group Methods Tests', () => {
  ...
  describe('Group Query Methods', () => {
    ...
    beforeEach(async () => {
      ...
      await User.findByIdAndUpdate(testUser._id, {
        $addToSet: { groupIds: testGroup._id }
      });
    });

    test('should find group by ID', async () => {
      const group = await findGroupById(testGroup._id);
    ...
    test('should find group by ID with specific projection', async () => {
      const group = await findGroupById(testGroup._id, { name: 1 });
    ...
    test('should find group by external ID', async () => {
      ...
      const group = await findGroupByExternalId('entra-id-12345', 'entra');
    ...
    test('should return null for non-existent external ID', async () => {
      const group = await findGroupByExternalId('non-existent-id', 'entra');
    ...
    test('should find groups by name pattern', async () => {
      ...
      await Group.create({ name: 'Test Entra Group', source: 'entra', idOnTheSource: 'entra-id-xyz' });
      
      // Search for all "Test" groups
      const testGroups = await findGroupsByNamePattern('Test');
      expect(testGroups).toHaveLength(3);
      
      // Search with source filter
      const localTestGroups = await findGroupsByNamePattern('Test', 'local');
      expect(localTestGroups).toHaveLength(2);
      
      const entraTestGroups = await findGroupsByNamePattern('Test', 'entra');
    ...
    test('should respect limit parameter in name search', async () => {
      ...
      const limitedGroups = await findGroupsByNamePattern('Numbered', null, 5);
    ...
    test('should find groups by member ID', async () => {
      ...
      const group3 = await Group.create({
        name: 'Third Group',
        source: 'local',
        memberIds: [new mongoose.Types.ObjectId()] // Different user
      });
      
      const userGroups = await findGroupsByMemberId(testUser._id);
  ...
  describe('Group Creation and Update Methods', () => {
    ...
    test('should upsert a group by external ID (create new)', async () => {
      ...
      const group = await upsertGroupByExternalId(
        groupData.idOnTheSource,
        'entra',
        { name: groupData.name }
      );
    ...
    test('should upsert a group by external ID (update existing)', async () => {
      ...
      // Update it
      const updatedGroup = await upsertGroupByExternalId(
        'existing-entra-id',
        'entra',
        { name: 'Updated Name' }
      );
  ...
  describe('User-Group Relationship Methods', () => {
    ...
    test('should add user to group', async () => {
      const result = await addUserToGroup(testUser1._id, testGroup._id);
      ...
      // User should have the group in groupIds
      expect(result.user.groupIds).toContainEqual(testGroup._id);
      
      // Group should have the user in memberIds
      expect(result.group.memberIds).toContainEqual(testUser1._id);
      
      // Verify in database
      const updatedUser = await User.findById(testUser1._id);
      const updatedGroup = await Group.findById(testGroup._id);
      
      expect(updatedUser.groupIds.map(id => id.toString())).toContain(testGroup._id.toString());
      expect(updatedGroup.memberIds.map(id => id.toString())).toContain(testUser1._id.toString());
    });

    test('should remove user from group', async () => {
      // First add the user to the group
      await addUserToGroup(testUser1._id, testGroup._id);
      
      // Then remove them
      const result = await removeUserFromGroup(testUser1._id, testGroup._id);
      ...
      // User should not have the group in groupIds
      expect(result.user.groupIds.map(id => id.toString())).not.toContain(testGroup._id.toString());
      
      // Group should not have the user in memberIds
      expect(result.group.memberIds.map(id => id.toString())).not.toContain(testUser1._id.toString());
      
      // Verify in database
      const updatedUser = await User.findById(testUser1._id);
      const updatedGroup = await Group.findById(testGroup._id);
      
      expect(updatedUser.groupIds.map(id => id.toString())).not.toContain(testGroup._id.toString());
      expect(updatedGroup.memberIds.map(id => id.toString())).not.toContain(testUser1._id.toString());
    });
    ...
    test('should get all groups for a user', async () => {
      ...
      await addUserToGroup(testUser1._id, group1._id);
      await addUserToGroup(testUser1._id, group2._id);
      
      // Get the user's groups
      const userGroups = await getUserGroups(testUser1._id);
    ...
    test('should get user principals', async () => {
      ...
      // Get user principals
      const principals = await getUserPrincipals(testUser1._id);
      ...
      expect(userPrincipal.principalId.toString()).toBe(testUser1._id.toString());
      ...
      expect(publicPrincipal.principalId).toBeNull();
    });

    test('should return empty array for non-existent user in getUserPrincipals', async () => {
      const nonExistentId = new mongoose.Types.ObjectId();
      const principals = await getUserPrincipals(nonExistentId);
  ...
  describe('Entra ID Synchronization', () => {
    ...
    test.skip('should sync Entra groups for a user (add new groups)', async () => {
      ...
      const result = await syncUserEntraGroups(testUser._id, entraGroups);
    ...
    test.skip('should sync Entra groups for a user (add and remove groups)', async () => {
      ...
      await User.findByIdAndUpdate(testUser._id, {
        $set: { groupIds: [existingGroup1._id, existingGroup2._id] }
      });
      ...
      expect(newGroup.memberIds.map(id => id.toString())).toContain(testUser._id.toString());
    });
    ...
    test('should throw error for non-existent user in syncUserEntraGroups', async () => {
      ...
      await expect(syncUserEntraGroups(nonExistentId, entraGroups)).rejects.toThrow('User not found');
    });
    ...
    test.skip('should preserve local groups when syncing Entra groups', async () => {
      ...
      expect(savedLocalGroup.memberIds.map(id => id.toString())).toContain(testUser._id.toString());
      ...
      expect(entraGroup.memberIds.map(id => id.toString())).toContain(testUser._id.toString());
    });
  });
});