...
import type { TStartupConfig, TPrincipal, TSelectedPrincipal } from 'librechat-data-provider';
import {
  Button,
  OGDialog,
  OGDialogTitle,
...
export default function ShareAgentEnhanced({
  ...
  const handleRoleChange = (tempId: string, newRole: string) => {
    setSelectedShares(
      selectedShares.map((s) => (s.tempId === tempId ? { ...s, accessRoleId: newRole } : s)),
    );
  };
    ...
    <OGDialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      ...
      <OGDialogContent className="max-h-[90vh] w-11/12 overflow-y-auto md:max-w-3xl">
        ...
        <div className="space-y-6 p-2">
          ...
          <PeoplePicker selectedShares={selectedShares} onSelectPrincipal={handleSelectPrincipal} />
          ...
          <div className="flex justify-end gap-3 border-t pt-4">
            ...
            <Button onClick={handleSave} disabled={isSubmitting} className="min-w-[80px]">
              {isSubmitting ? (
                <div className="flex items-center gap-2">
