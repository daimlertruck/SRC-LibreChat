...
import { useSharePointPicker, useLocalize, useSharePointToken } from '~/hooks';
import { useGetStartupConfig } from '~/data-provider';
import SharePointSitesBrowser from './SharePointSitesBrowser';
import type { SharePointBatchProgress } from '~/data-provider';
import type { CustomCommand } from '~/hooks/Files/useSharePointPicker';
...
type DialogView = 'picker' | 'sites-browser';
...
export default function SharePointPickerDialog({
  ...
  const [currentView, setCurrentView] = useState<DialogView>('picker');
  const [selectedEntryWeb, setSelectedEntryWeb] = useState<string>();
  const localize = useLocalize();

  // Internal custom commands - single command to open sites browser
  const customCommands: CustomCommand[] = [
    {
      key: 'browse-all-sites',
      label: localize('com_files_sharepoint_all_sites') || 'All Sites',
      icon: 'SharepointAppIcon16',
      action: 'custom' as const,
      handler: () => setCurrentView('sites-browser'),
    },
  ];
  ...
  const { token: sharePointToken } = useSharePointToken({
    enabled: isOpen,
  });

  const handleSiteSelected = (siteUrl: string) => {
    setSelectedEntryWeb(siteUrl);
    setCurrentView('picker');
  };

  const handleOpenChange = (open: boolean) => {
    if (!open) {
      if (currentView === 'sites-browser') {
        setCurrentView('picker');
        return;
      } else {
        closeSharePointPicker();
        setSelectedEntryWeb(undefined);
      }
    }
    onOpenChange(open);
  };
  ...
  }, [containerNode, isOpen, selectedEntryWeb]);
  return (
    <>
      {/* SharePoint File Picker */}
      {
        <OGDialog open={isOpen} onOpenChange={handleOpenChange}>
          <OGDialogPortal>
            ...
            <OGDialogContent
              className="bg-#F5F5F5 sharepoint-picker-bg fixed left-1/2 top-1/2 z-50 h-[680px] max-h-[90vh] w-[1200px] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-lg border p-2 shadow-lg focus:outline-none"
            ...
            >
              ...
              {currentView === 'sites-browser' && (
                <SharePointSitesBrowser
                  onSiteSelected={handleSiteSelected}
                  sharePointBaseUrl={startupConfig?.sharePointBaseUrl || ''}
                  accessToken={sharePointToken?.access_token || ''}
                />
              )}
            </OGDialogContent>
          </OGDialogPortal>
